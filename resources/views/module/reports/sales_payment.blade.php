<x-app-layout title="Sales Payments Report">
    <div x-data="salesPaymentReport()" x-cloak>
                
        <!-- HEADER & ACTIONS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                 <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Sales & Payment Report <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Financial History</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Sales & Payment</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-card rounded-2xl border border-slate-100 dark:border-dark-border p-5 mb-6 shadow-sm">
            ...
            <div class="flex justify-end gap-2">
                ...
                <button @click="fetchData()" :disabled="isLoading" class="px-6 py-2 bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200/50 dark:shadow-none flex items-center gap-2 disabled:opacity-50">
                    ...
                    <span x-text="isLoading ? 'Loading...' : 'Show Records'"></span>
                </button>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-card rounded-3xl border border-slate-100 dark:border-dark-border overflow-hidden shadow-sm">
            ...
            <!-- Customer Summary Section (Refined) -->
            <div class="px-6 py-4 border-b border-slate-50 dark:border-dark-border bg-slate-50/20">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <div>
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-0.5 italic">Name</span>
                        <span class="text-[11px] font-bold text-slate-700 dark:text-white" x-text="customerInfo.name || 'Not Selected'"></span>
                    </div>
                    <div>
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-0.5 italic">Mobile</span>
                        <span class="text-[11px] font-bold text-slate-700 dark:text-white" x-text="customerInfo.mobile"></span>
                    </div>
                    <div>
                        <span class="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-0.5 italic">Address</span>
                        <span class="text-[11px] font-bold text-slate-500 dark:text-slate-400" x-text="customerInfo.address || 'Not Provided'"></span>
                    </div>
                    <div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-dark-border">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Date/Invoice</th>
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Description</th>
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Bill Amt</th>
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center">Receive</th>
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Balance</th>
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-dark-border">
                        <tr x-show="filteredRecords.length === 0">
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <span class="text-[10px] font-black uppercase tracking-widest opacity-50 italic">No Payment Records Found</span>
                                </div>
                            </td>
                        </tr>
                        <template x-for="record in filteredRecords" :key="record.id">
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                ...
                                <td class="px-6 py-2.5 text-[10px] font-bold text-slate-500" x-text="record.description"></td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-slate-700 dark:text-white" x-text="record.billAmt"></td>
                                <td class="px-6 py-2.5 text-center font-mono text-[11px] font-black text-emerald-600" x-text="record.receive"></td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-rose-500" x-text="record.total"></td>
                                <td class="px-6 py-2.5 text-center">
                                     <div x-data="{ open: false }" class="relative inline-block text-left">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-36 origin-top-right rounded-xl bg-white dark:bg-dark-card shadow-2xl border border-slate-100 dark:border-dark-border z-20 overflow-hidden text-left" x-cloak>
                                            <div class="py-1">
                                                ...
                                                <a href="#" class="block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest border-t border-slate-50 dark:border-dark-border">Print</a>
                                            </div>
                                        </div>
                                     </div>
                                </td>
            ...
            return {
                ...
                get filteredRecords() {
                    ...
                    return this.records.filter(r => 
                        ...
                        (r.refBill && r.refBill.toLowerCase().includes(this.searchTerm.toLowerCase()))
                    );
                },
                ...
                resetFilters() {
                    ...
                    this.customerId = '';
                    this.records = [];
                    this.customerInfo = { name: '', mobile: '', address: '', previousDue: '0.00' };
                    this.totalBillAmt = '0.00';
                ...
                fetchData() {
                    ...
                    fetch(`{{ route('reports.sales_payment_data') }}?start_date=${this.fromDate}&end_date=${this.toDate}&customer_id=${this.customerId}`)
                        .then(res => res.json())
                        .then(data => {
                            if (data.status === 'success') {
                                this.records = data.records || [];
                                this.customerInfo = data.customerInfo || { name: '', mobile: '', address: '', previousDue: '0.00' };
                                this.totalBillAmt = (parseFloat(data.totalBillAmt) || 0).toFixed(2);
                                this.totalReceive = (parseFloat(data.totalReceive) || 0).toFixed(2);
                            } else {
