<x-app-layout title="Sales GST Report">
    <div x-data="salesGstReport()" x-cloak>
                
        <!-- HEADER & ACTIONS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                 <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Sales GST Report <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Tax Analysis</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Sales GST Report</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-card rounded-2xl border border-slate-100 dark:border-dark-border p-5 mb-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                ...
                <div class="relative group">
                    ...
                    <x-searchable-select :options="$customers" labelKey="customer_name" valueKey="id" subtextKey="mobile" emptyOption="-All-" emptyValue="all" placeholder="Select Customer" model="customerId" />
                </div>
            </div>
        ...
        <div class="bg-white dark:bg-dark-card rounded-3xl border border-slate-100 dark:border-dark-border overflow-hidden shadow-sm">
            ...
            <div class="px-4 py-3 border-b border-slate-50 dark:border-dark-border bg-slate-50/20 dark:bg-white/5 flex flex-wrap justify-between items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                    <x-report-export-buttons :route="route('reports.sales_gst_data')" />
                    <div class="relative group">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-dark-border">
                        <template x-for="record in filteredRecords" :key="record.id">
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                ...
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-slate-700 dark:text-white"><x-money value="parseFloat(record.taxable.replace(/,/g,''))" symbol="{{ $currencySymbol ?? '' }}" /></td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-emerald-600"><x-money value="parseFloat(record.tax.replace(/,/g,''))" symbol="{{ $currencySymbol ?? '' }}" /></td>
                    ...
                    <tfoot class="bg-slate-50/80 dark:bg-slate-800/80 border-t border-slate-200">
                        <tr class="font-black text-slate-700 dark:text-slate-200">
                            ...
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right font-mono"><x-money value="records.reduce((acc, r) => acc + parseFloat(r.taxable.replace(/,/g,'')), 0)" symbol="{{ $currencySymbol ?? '' }}" /></td>
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right text-emerald-600 font-mono"><x-money value="records.reduce((acc, r) => acc + parseFloat(r.tax.replace(/,/g,'')), 0)" symbol="{{ $currencySymbol ?? '' }}" /></td>
        ...
        function salesGstReport() {
            return {
                ...
                fetchData() {
                    ...
                    fetch(`{{ route('reports.sales_gst_data') }}?start_date=${this.fromDate}&end_date=${this.toDate}&customer_id=${this.customerId}`)
                        .then(res => res.json())
                        .then(data => {
