<x-app-layout title="Expense Report">
    <div x-data="expenseReportJS()" x-cloak>
                
        <!-- HEADER & ACTIONS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                 <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Expense Report <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Cost Analysis</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Expense Report</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-card rounded-2xl border border-slate-100 dark:border-dark-border p-5 mb-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div class="relative group">
                    <label class="absolute -top-1.5 left-3 px-1 bg-white dark:bg-dark-card text-[9px] font-black uppercase text-slate-400 tracking-widest z-10">Category Name</label>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-dark-border">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Expense Details</th>
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Category & Purpose</th>
                            ...
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Owner</th>
                        ...
                        <tr x-show="!isLoading && filteredRecords.length === 0" x-cloak>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <span class="text-[10px] font-black uppercase tracking-widest opacity-50 italic">No Expense Records Found</span>
                                </div>
                            </td>
                        </tr>
                                ...
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-col">
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest" x-text="record.reference"></span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest" x-text="record.expenseFor"></span>
                                    </div>
                                </td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-rose-500" x-text="'{{ $currencySymbol }}' + record.amount"></td>
                                <td class="px-6 py-2.5 min-w-[150px]">
                                    <span class="text-[9px] font-bold text-slate-400 italic" x-text="record.note"></span>
                                </td>
                                <td class="px-6 py-2.5 text-right">
                                    <span class="text-[10px] font-bold text-slate-500" x-text="record.createdBy"></span>
                                </td>
                                <td class="px-6 py-2.5 text-center">
                                     <div x-data="{ open: false }" class="relative inline-block text-left">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-36 origin-top-right rounded-xl bg-white dark:bg-dark-card shadow-2xl border border-slate-100 dark:border-dark-border z-20 overflow-hidden text-left" x-cloak>
                                            <div class="py-1">
                                                <a href="#" class="block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest">View Receipt</a>
                                                <a href="#" class="block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest border-t border-slate-50 dark:border-dark-border">Edit Entry</a>
                                            </div>
                                        </div>
                                     </div>
                                </td>
                    ...
                    <tfoot class="bg-slate-50/80 dark:bg-slate-800/80 border-t border-slate-200">
                        <tr class="font-black text-slate-700 dark:text-slate-200">
                            <td colspan="3" class="px-6 py-3 text-right uppercase text-[9px] tracking-widest text-slate-500 italic">Total Summary</td>
                             <td class="px-6 py-3 text-[11px] tabular-nums text-right text-rose-500 font-mono">{{ $currencySymbol }}<span x-text="records.reduce((acc, r) => acc + parseFloat(r.amount.replace(/,/g, '')), 0).toFixed(2)"></span></td>
                            <td colspan="3"></td>
                        </tr>
                    </tfoot>
                </table>
            ...
            return {
                ...
                get filteredRecords() {
                    ...
                    return this.records.filter(r => 
                        (r.code && r.code.toLowerCase().includes(this.searchTerm.toLowerCase())) || 
                        (r.category && r.category.toLowerCase().includes(this.searchTerm.toLowerCase())) ||
                        (r.expenseFor && r.expenseFor.toLowerCase().includes(this.searchTerm.toLowerCase())) ||
                        (r.reference && r.reference.toLowerCase().includes(this.searchTerm.toLowerCase()))
                    );
                },
                ...
                fetchData() {
                    this.isLoading = true;
                    let url = `{{ route('reports.expense_data') }}?start_date=${this.fromDate}&end_date=${this.toDate}&category_id=${this.categoryId}`;
