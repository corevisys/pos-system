<x-app-layout title="Purchase Payments Report">
    <div x-data="purchasePaymentsReportJS()" x-cloak>
                
        <!-- HEADER & ACTIONS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                 <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Purchase Payments Report <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Outbound Payments</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Purchase Payments Report</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-card rounded-2xl border border-slate-100 dark:border-dark-border p-5 mb-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                <div class="relative group">
                    ...
                    <x-searchable-select :options="$suppliers" labelKey="supplier_name" valueKey="id" emptyOption="-All-" emptyValue="all" placeholder="Select Supplier" model="supplierId" />
                </div>

                <div class="relative group">
                    <label class="absolute -top-1.5 left-3 px-1 bg-white dark:bg-dark-card text-[9px] font-black uppercase text-slate-400 tracking-widest z-10">Payment Type</label>
                    <div class="relative">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-dark-border">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Payment Info</th>
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Supplier Details</th>
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-dark-border">
                        ...
                        <tr x-show="!isLoading && filteredRecords.length === 0" x-cloak>
                            <td colspan="7" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center gap-2">
                                    <svg class="w-8 h-8 opacity-20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 114 0z"></path></svg>
                        ...
                        <template x-for="record in filteredRecords" :key="record.id">
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                ...
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-col">
                                        <span class="text-[11px] font-bold text-slate-800 dark:text-white" x-text="record.supplier"></span>
                                        <span class="text-[9px] font-bold text-slate-400 uppercase tracking-widest" x-text="record.supplierId"></span>
                                    </div>
                                </td>
                                <td class="px-6 py-2.5 text-center">
                                ...
                                <td class="px-6 py-2.5 text-center">
                                     <div x-data="{ open: false }" class="relative inline-block text-left">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-36 origin-top-right rounded-xl bg-white dark:bg-dark-card shadow-2xl border border-slate-100 dark:border-dark-border z-20 overflow-hidden text-left" x-cloak>
                                            <div class="py-1">
                                                <a href="#" class="block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest">Receipt</a>
                                                <a href="#" class="block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest border-t border-slate-50 dark:border-dark-border text-rose-600">Delete</a>
                                            </div>
                                        </div>
                                     </div>
                                </td>
    ...
    <script>
        function purchasePaymentsReportJS() {
            return {
                supplierId: 'all',
                paymentType: 'all',
                userId: 'all',
                ...
                get filteredRecords() {
                    ...
                    return this.records.filter(r => 
                        ...
                        (r.supplierId && r.supplierId.toLowerCase().includes(search)) ||
                        (r.purchase_code && r.purchase_code.toLowerCase().includes(search))
                    );
                },
                ...
                fetchData() {
                    this.isLoading = true;
                    let url = `{{ route('reports.purchase_payments_data') }}?start_date=${this.fromDate}&end_date=${this.toDate}&supplier_id=${this.supplierId}&payment_type=${this.paymentType}&user_id=${this.userId}`;
                    ...
                    fetch(url)
                        ...
                        .catch(err => {
                            console.error(err);
                            showError('Failed to fetch purchase payments report');
                        })
                        .finally(() => {
