<x-app-layout title="Supplier Items Report">
    <div x-data="supplierItemsReport()" x-cloak>
                
        <!-- HEADER & ACTIONS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                 <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Supplier Items <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Inventory List</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Supplier Items Report</span>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-dark-border">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Item Details</th>
                        ...
                        <tr x-show="!isLoading && filteredRecords.length === 0" x-cloak>
                            <td colspan="6" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <span class="text-[10px] font-black uppercase tracking-widest opacity-50 italic">No Items Found</span>
                                </div>
                            </td>
                        </tr>
                            ...
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                ...
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-col">
                                        <span class="text-[11px] font-bold text-slate-700 dark:text-slate-200" x-text="record.item"></span>
                                        <span class="text-[9px] font-black text-primary-600 uppercase tracking-tight" x-text="record.code"></span>
                                    </div>
                                </td>
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-col">
                                        <span class="text-[10px] font-black text-slate-700 dark:text-slate-200" x-text="record.invoice"></span>
                                        <div class="flex items-center gap-1.5 mt-0.5">
                                            <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest" x-text="record.date"></span>
                                ...
                                <td class="px-6 py-2.5 text-center">
                                     <div x-data="{ open: false }" class="relative inline-block text-left">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-36 origin-top-right rounded-xl bg-white dark:bg-dark-card shadow-2xl border border-slate-100 dark:border-dark-border z-20 overflow-hidden text-left" x-cloak>
                                            <div class="py-1">
                                                <a href="#" class="block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest">Item Details</a>
        ...
        function supplierItemsReport() {
            return {
                ...
                fetchData() {
                    ...
                    fetch(`{{ route('reports.supplier_items_data') }}?item_id=${this.itemId}&supplier_id=${this.supplierId}`)
                        .then(res => res.json())
                        .then(data => {
