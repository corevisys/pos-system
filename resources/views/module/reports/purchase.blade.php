<x-app-layout title="Purchase Report">
    <div x-data="purchaseReportJS()" x-cloak>
                
        <!-- HEADER & ACTIONS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                 <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Purchase Report <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Procurement Summary</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Purchase Report</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-card rounded-2xl border border-slate-100 dark:border-dark-border p-5 mb-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                ...
                <div class="relative group">
                    ...
                    <x-searchable-select :options="$suppliers" labelKey="supplier_name" valueKey="id" emptyOption="-All Suppliers-" emptyValue="all" placeholder="Select Supplier" model="supplierId" />
                </div>
             ...
             <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="flex items-center gap-2">
                    <label class="relative inline-flex items-center cursor-pointer">
                        <input type="checkbox" x-model="viewAccountPayable" class="sr-only peer">
                        <div class="w-9 h-5 bg-slate-200 peer-focus:outline-none rounded-full peer dark:bg-slate-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-4 after:w-4 after:transition-all dark:border-gray-600 peer-checked:bg-primary-600"></div>
                        <span class="ml-2 text-[10px] font-black uppercase tracking-widest text-slate-500">View Account Payable</span>
                    </label>
                </div>
                                ...
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-col">
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest" x-text="record.warehouse"></span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest" x-text="record.supplierId"></span>
                                    </div>
                                </td>
                                     ...
                                     <div x-data="{ open: false }" class="relative inline-block text-left">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-36 origin-top-right rounded-xl bg-white dark:bg-dark-card shadow-2xl border border-slate-100 dark:border-dark-border z-20 overflow-hidden text-left" x-cloak>
                                            <div class="py-1">
                                                ...
                                                <a href="#" class="block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest border-t border-slate-50 dark:border-dark-border">Purchase PDF</a>
                                            </div>
                                        </div>
                                     </div>
                                </td>
            ...
            return {
                ...
                get filteredRecords() {
                    ...
                    return this.records.filter(r => 
                        r.invoice.toLowerCase().includes(this.searchTerm.toLowerCase()) || 
                        r.supplier.toLowerCase().includes(this.searchTerm.toLowerCase()) ||
                        r.supplierId.toLowerCase().includes(this.searchTerm.toLowerCase())
                    );
                },
                ...
                resetFilters() {
                    ...
                    this.viewAccountPayable = false;
                    this.records = [];
                },

                fetchData() {
                    this.isLoading = true;
                    let url = `{{ route('reports.purchase_data') }}?start_date=${this.fromDate}&end_date=${this.toDate}&warehouse_id=${this.warehouseId}&supplier_id=${this.supplierId}&view_account_payable=${this.viewAccountPayable}`;
