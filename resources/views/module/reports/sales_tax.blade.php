        ...
        <div class="bg-white dark:bg-dark-card rounded-3xl border border-slate-100 dark:border-dark-border overflow-hidden shadow-sm">
            ...
            <div class="px-4 py-3 border-b border-slate-50 dark:border-dark-border bg-slate-50/20 dark:bg-white/5 flex flex-wrap justify-between items-center gap-4">
                ...
                <div class="flex items-center gap-2">
                    <x-report-export-buttons :route="route('reports.sales_tax_data')" />
                    <div class="relative group">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-dark-border">
                        ...
                        <template x-for="record in filteredRecords" :key="record.id">
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                ...
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-slate-700 dark:text-white"><x-money value="parseFloat(record.rate.replace(/,/g,''))" symbol="{{ $currencySymbol ?? '' }}" /></td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-emerald-600"><x-money value="parseFloat(record.taxAmount.replace(/,/g,''))" symbol="{{ $currencySymbol ?? '' }}" /></td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-slate-800 dark:text-white"><x-money value="parseFloat(record.total.replace(/,/g,''))" symbol="{{ $currencySymbol ?? '' }}" /></td>
                    ...
                    <tfoot class="bg-slate-50/80 dark:bg-slate-800/80 border-t border-slate-200">
                        <tr class="font-black text-slate-700 dark:text-slate-200">
                            ...
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right font-mono"><x-money value="records.reduce((acc, r) => acc + parseFloat(r.rate.replace(/,/g,'')), 0)" symbol="{{ $currencySymbol ?? '' }}" /></td>
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right text-emerald-600 font-mono"><x-money value="records.reduce((acc, r) => acc + parseFloat(r.taxAmount.replace(/,/g,'')), 0)" symbol="{{ $currencySymbol ?? '' }}" /></td>
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right text-slate-800 dark:text-white font-mono"><x-money value="records.reduce((acc, r) => acc + parseFloat(r.total.replace(/,/g,'')), 0)" symbol="{{ $currencySymbol ?? '' }}" /></td>
                            </tr>
                    </tfoot>
                </table>
            </div>
