<x-app-layout title="GSTR-1 Report">
    <div x-data="gstr1Report()" x-cloak>
                
        <!-- HEADER & ACTIONS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                 <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">GSTR-1 Report <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Sales Tax Return</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">GSTR-1 Report</span>
                </div>
            </div>
        </div>
                ...
                <table class="w-full text-left border-collapse">
                            ...
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                ...
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-col">
                                        ...
                                        <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest" x-text="record.gst || 'No GST'"></span>
                                    </div>
                                </td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-slate-700 dark:text-white" x-text="'{{ $currencySymbol }}' + record.rate"></td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-emerald-600" x-text="'{{ $currencySymbol }}' + record.tax"></td>
                                     ...
                                     <div x-data="{ open: false }" class="relative inline-block text-left">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-36 origin-top-right rounded-xl bg-white dark:bg-dark-card shadow-2xl border border-slate-100 dark:border-dark-border z-20 overflow-hidden text-left" x-cloak>
                                            <div class="py-1">
                                                ...
                                                <a href="#" class="block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest border-t border-slate-50 dark:border-dark-border">Invoice</a>
                                            </div>
                                        </div>
                                     </div>
                                </td>
                    ...
                    <tfoot class="bg-slate-50/80 dark:bg-slate-800/80 border-t border-slate-200">
                        <tr class="font-black text-slate-700 dark:text-slate-200">
                            ...
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right font-mono">{{ $currencySymbol }}<span x-text="records.reduce((acc, r) => acc + parseFloat(r.rate.replace(/,/g, '')), 0).toFixed(2)"></span></td>
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right text-emerald-600 font-mono">{{ $currencySymbol }}<span x-text="records.reduce((acc, r) => acc + parseFloat(r.tax.replace(/,/g, '')), 0).toFixed(2)"></span></td>
        ...
        function gstr1Report() {
            return {
                ...
                fetchData() {
                    ...
                    fetch(`{{ route('reports.gstr1_data') }}?start_date=${this.fromDate}&end_date=${this.toDate}`)
                        .then(res => res.json())
                        .then(data => {
