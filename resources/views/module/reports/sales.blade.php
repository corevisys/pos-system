<x-app-layout title="Sales Report">
    <div x-data="salesReportJS()" x-cloak>
                
        <!-- HEADER & ACTIONS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
             <div>
                 <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Sales Report <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Transaction Summary</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Sales Report</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-dark-card rounded-2xl border border-slate-100 dark:border-dark-border p-5 mb-6 shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                ...
                <div class="relative group">
                    <label class="absolute -top-1.5 left-3 px-1 bg-white dark:bg-dark-card text-[9px] font-black uppercase text-slate-400 tracking-widest z-10">Customer</label>
                ...
                <div class="relative group">
                    <label class="absolute -top-1.5 left-3 px-1 bg-white dark:bg-dark-card text-[9px] font-black uppercase text-slate-400 tracking-widest z-10">Payment Status</label>
                    <div class="relative">
                        <select x-model="paymentStatus" class="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-dark-border rounded-xl py-2 px-3 focus:ring-1 focus:ring-primary-500 focus:border-primary-500 text-[11px] font-bold appearance-none transition-all cursor-pointer outline-none">
                ...
                <table class="w-full text-left border-collapse">
                    <thead class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-dark-border">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Customer</th>
                            ...
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Paid ({{ $currencySymbol }})</th>
                            <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-right">Due ({{ $currencySymbol }})</th>
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-dark-border">
                        ...
                        <tr x-show="!isLoading && filteredRecords.length === 0" x-cloak>
                            <td colspan="8" class="px-6 py-12 text-center text-slate-400">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <span class="text-[10px] font-black uppercase tracking-widest opacity-50 italic">No Sales Found</span>
                                </div>
                            </td>
                        </tr>
                        <template x-for="record in filteredRecords" :key="record.id">
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                ...
                                <td class="px-6 py-2.5">
                                    <div class="flex flex-col">
                                        <span class="text-[10px] font-black text-slate-700 dark:text-slate-200" x-text="record.customer"></span>
                                        <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest" x-text="record.location"></span>
                                    </div>
                                </td>
                                <td class="px-6 py-2.5 text-center">
                                    <span :class="{
                                        'bg-emerald-50 text-emerald-600 border-emerald-100': record.status === 'Paid',
                                        'bg-amber-50 text-amber-600 border-amber-100': record.status === 'Partial',
                                        'bg-rose-50 text-rose-600 border-rose-100': record.status === 'Due'
                                    }" class="px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-widest border" x-text="record.status"></span>
                                </td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-slate-700 dark:text-white" x-text="'{{ $currencySymbol }}' + record.total"></td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-emerald-600" x-text="'{{ $currencySymbol }}' + record.paid"></td>
                                <td class="px-6 py-2.5 text-right font-mono text-[11px] font-black text-rose-500" x-text="'{{ $currencySymbol }}' + record.due"></td>
                                <td class="px-6 py-2.5 text-center">
                                     <div x-data="{ open: false }" class="relative inline-block text-left">
                                        ...
                                        <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-36 origin-top-right rounded-xl bg-white dark:bg-dark-card shadow-2xl border border-slate-100 dark:border-dark-border z-20 overflow-hidden text-left" x-cloak>
                                            <div class="py-1">
                                                ...
                                                <a href="#" class="block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest border-t border-slate-50 dark:border-dark-border">Invoice Bill</a>
                                            </div>
                                        </div>
                                     </div>
                                </td>
                    ...
                    <tfoot class="bg-slate-50/80 dark:bg-slate-800/80 border-t border-slate-200">
                        <tr class="font-black text-slate-700 dark:text-slate-200">
                            ...
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right font-mono">{{ $currencySymbol }}<span x-text="records.reduce((acc, r) => acc + parseFloat(r.total.replace(/,/g, '')), 0).toFixed(2)"></span></td>
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right text-emerald-600 font-mono">{{ $currencySymbol }}<span x-text="records.reduce((acc, r) => acc + parseFloat(r.paid.replace(/,/g, '')), 0).toFixed(2)"></span></td>
                            <td class="px-6 py-3 text-[11px] tabular-nums text-right text-rose-500 font-mono">{{ $currencySymbol }}<span x-text="records.reduce((acc, r) => acc + parseFloat(r.due.replace(/,/g, '')), 0).toFixed(2)"></span></td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
        ...
        function salesReportJS() {
            return {
                ...
                fetchData() {
                    ...
                    fetch(`{{ route('reports.sales_data') }}?start_date=${this.fromDate}&end_date=${this.toDate}&customer_id=${this.customerId}&payment_status=${this.paymentStatus}`)
                        .then(res => res.json())
                        .then(data => {
