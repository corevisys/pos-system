<x-app-layout title="Edit Coupon">
    <div>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">Edit Discount Coupon</h1>
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <a href="{{ route('coupons.master') }}" class="hover:text-primary transition-colors text-[10px] font-bold text-text-muted">Coupons</a>
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-text-secondary text-[10px] font-bold uppercase tracking-widest">Edit Coupon</span>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-3xl mx-auto" x-data="{
            ...
            <div class="card p-6">
                <div class="flex items-center gap-3 mb-6 border-b border-border-light dark:border-dark-border pb-4">
                    ...
                    <div>
                        <h2 class="text-sm font-black text-text-primary dark:text-dark-text uppercase tracking-wider">Edit Master Coupon</h2>
                        <p class="text-[10px] text-text-muted font-bold uppercase tracking-widest mt-0.5">Update parameters and status for campaign code: <span class="font-mono text-primary font-black">{{ $coupon->code }}</span></p>
                    </div>
                </div>
                ...
                <form action="{{ route('coupons.update', $coupon->id) }}" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 gap-y-4">
                        ...
                        <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-6">
                            ...
                            <div class="flex-1">
                                <input type="text" name="name" value="{{ old('name', $coupon->name) }}" placeholder="e.g., Summer Sale, Eid Special" class="input-base" required>
                            </div>
                        </div>
                        ...
                        <!-- Expire Date -->
                        <div class="flex flex-col md:flex-row md:items-center gap-2 md:gap-6">
                            <label class="w-full md:w-40 text-[10px] font-black uppercase text-text-muted tracking-widest md:text-right">Expiry Date <span class="text-rose-500">*</span></label>
                            <div class="flex-1 relative">
                                <input type="date" name="expire_date" value="{{ old('expire_date', $coupon->expire_date ? \Carbon\Carbon::parse($coupon->expire_date)->format('Y-m-d') : '') }}" class="input-base" required>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-col md:flex-row items-start gap-2 md:gap-6">
                            ...
                            <div class="flex-1">
                                <textarea name="description" rows="3" placeholder="Campaign details or terms..." class="input-base">{{ old('description', $coupon->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3 pt-4 border-t border-border-light dark:border-dark-border mt-6">
                        <a href="{{ route('coupons.master') }}" class="btn-secondary">
                            Close
                        </a>
