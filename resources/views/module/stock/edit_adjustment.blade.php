<x-app-layout title="Edit Stock Adjustment">
    <div x-data="adjustmentForm()">
        
        <!-- HEADER -->
        <div class="flex flex-wrap items-center justify-between gap-3 mb-3 px-1 text-black">
            <div>
                <h1 class="text-lg font-black tracking-tight text-slate-800 dark:text-white">Edit Stock Adjustment</h1>
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Update existing manual inventory adjustment</p>
            </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-3">
                                    ...
                                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                                        ...
                                        <td class="px-4 py-2 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                ...
                                                <button @click="updateQty(index, 1)" class="w-6 h-6 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-400 hover:bg-emerald-500 hover:text-white transition-all shadow-sm">
            ...
            <div class="lg:col-span-4 flex flex-col gap-3">
                ...
                <div class="bg-slate-900 rounded-[2rem] p-6 shadow-2xl text-white relative overflow-hidden group">
                    ...
                    <div class="relative space-y-4">
                        <div class="flex justify-between items-center border-b border-white/10 pb-3">
                            <h3 class="text-[9px] font-black uppercase tracking-widest text-slate-400">Update Summary</h3>
                        ...
                        <div class="space-y-1 text-center py-2">
                            <p class="text-[9px] font-black uppercase tracking-[0.2em] text-emerald-400">Total Adjustment Quantity</p>
                ...
                <p class="text-[9px] text-center font-bold text-slate-400 uppercase tracking-widest px-8">
                    Note: Updating this adjustment will recalculate the warehouse stock based on the new differences.
                </p>
            </div>
        </div>
        ...
        document.addEventListener('alpine:init', () => {
             Alpine.data('adjustmentForm', () => ({
                form: {
                    ...
                    adjustment_note: `{!! addslashes($adjustment->adjustment_note) !!}`,
                    adjustment_date: '{{ $adjustment->adjustment_date }}',
                    items: @json($adjustmentItems)
                },
                searchQuery: '',
                searchResults: [],
                ...
                init() {
                    this.$watch('form.warehouse_id', (value, oldValue) => {
                ...
                async submitForm() {
                    ...
                    try {
                        const response = await fetch(`{{ route('stock.adjustment.update', $adjustment->id, false) }}`, {
                            method: 'POST',
                            headers: {
                        ...
                        if (result.success) {
                            showSuccess(result.message || 'Stock Adjustment updated successfully!');
                            setTimeout(() => { window.location.href = "{{ route('stock.adjustment') }}"; }, 800);
                        } else {
                            showError(result.message || result.error || 'Failed to update adjustment.');
                        }
                    } catch (error) {
