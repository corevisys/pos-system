<x-app-layout title="Edit Stock Transfer">
    <div x-data="transferForm()">
        
        <!-- HEADER -->
        <div class="flex flex-wrap items-center justify-between gap-3 mb-3 px-1 text-black">
            <div>
                <h1 class="text-lg font-black tracking-tight text-slate-800 dark:text-white">Edit Stock Transfer</h1>
                <p class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Modify existing inventory movement between warehouses</p>
            </div>
            <a href="{{ route('stock.transfer') }}" class="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2">
                ...
                <div class="bg-white dark:bg-dark-card p-2 md:p-3 rounded-2xl border border-slate-100 dark:border-dark-border shadow-sm">
                    <div class="relative group" @click.away="searchResults = []">
                        ...
                        <div x-show="searchResults.length > 0" 
                             ...
                             x-transition:enter-end="opacity-100 translate-y-0 text-black">
                            <template x-for="item in searchResults" :key="item.id">
                                <div @click="addItem(item)" class="px-4 py-2 hover:bg-slate-50 dark:hover:bg-slate-800 cursor-pointer transition-colors border-b border-slate-50 dark:border-dark-border last:border-none flex items-center justify-between gap-3 italic font-black uppercase tracking-widest">
                ...
                <div class="bg-white dark:bg-dark-card rounded-2xl border border-slate-100 dark:border-dark-border overflow-hidden flex-1 shadow-sm">
                    <div class="overflow-x-auto custom-scrollbar min-h-[400px]">
                        <table class="w-full text-left">
                            <thead class="bg-slate-50 dark:bg-slate-800/50 sticky top-0 z-10 shadow-sm border-b border-slate-100 dark:border-dark-border text-black">
                                <tr>
                                    <th class="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap italic">Item Information</th>
                                    <th class="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center whitespace-nowrap italic">Source Stock</th>
                                    <th class="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest text-center whitespace-nowrap italic">Transfer Qty</th>
                                            ...
                                            <div class="flex flex-col">
                                                ...
                                                <div class="flex items-center gap-2 mt-0.5">
                                                    ...
                                                    <template x-if="item.is_serialized">
                                                        <button @click="openSerialModal(index)" class="px-1.5 py-0.5 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 rounded text-[8px] font-black uppercase tracking-widest border border-emerald-100 dark:border-emerald-500/20 hover:bg-emerald-100 transition-all flex items-center gap-1 leading-none shadow-sm">
                        ...
                        <div x-show="form.items.length === 0" class="flex flex-col items-center justify-center py-20 px-6 text-center">
                            <div class="w-16 h-16 bg-slate-50 dark:bg-slate-800/50 rounded-full flex items-center justify-center mb-4 text-slate-200 dark:text-slate-700 shadow-inner">
                            ...
                            <h3 class="text-[11px] font-black text-slate-400 uppercase tracking-widest italic">No Items Loaded</h3>
                            <p class="text-[10px] text-slate-400 mt-1 font-medium">Add items to transfer</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="lg:col-span-4 flex flex-col gap-3">
                ...
                <div class="bg-white dark:bg-dark-card p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-dark-border flex flex-col gap-4">
                    ...
                    <div class="grid grid-cols-1 gap-3">
                        <div>
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest block mb-1 pl-1">Transfer Date *</label>
                        ...
                        <div>
                            <label class="text-[9px] font-black uppercase text-slate-400 tracking-widest block mb-1 pl-1 italic">Transfer Note</label>
                            <textarea x-model="form.note" rows="2" placeholder="Update notes..." 
                                      class="w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-dark-border rounded-xl py-2 px-4 text-[11px] font-bold text-slate-700 dark:text-slate-300 focus:ring-1 focus:ring-primary-500 outline-none transition-all resize-none placeholder:text-slate-300 shadow-inner italic"></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-slate-900 rounded-[2rem] p-6 shadow-2xl text-white relative overflow-hidden group">
                    <div class="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-3xl -mr-16 -mt-16 pointer-events-none group-hover:bg-emerald-500/20 transition-all duration-700"></div>
                    
                    <div class="relative space-y-4">
                        <div class="flex justify-between items-center border-b border-white/10 pb-3">
                            <h3 class="text-[9px] font-black uppercase tracking-widest text-slate-400 italic">Update Summary</h3>
                        ...
                        <div class="space-y-1 text-center py-2">
                            <p class="text-[9px] font-black uppercase tracking-[0.2em] text-emerald-400 italic font-black">Total Updated Qty</p>
                            <p class="text-5xl font-black tracking-tighter" x-text="totalQuantities"></p>
                        </div>

                        <div class="pt-1">
                            <button @click="submitForm" class="w-full py-4 bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all transform active:scale-[0.98] shadow-lg shadow-emerald-500/40 border border-emerald-400/10 flex items-center justify-center gap-2">
                ...
                <p class="text-[9px] text-center font-bold text-slate-400 uppercase tracking-widest px-8 italic">
                    Note: Updating will revert the old stock movement and apply the new one.
                </p>
            </div>
        </div>
        ...
        <div x-show="serialModalOpen" 
             class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/60 backdrop-blur-md p-4 text-black"
             x-cloak
            ...
            <div class="bg-white dark:bg-dark-card w-full max-w-lg rounded-[2.5rem] shadow-2xl p-8 transform transition-all border border-slate-100 dark:border-dark-border"
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h2 class="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-widest">Serials</h2>
                ...
                <div class="max-h-[50vh] overflow-y-auto pr-2 custom-scrollbar space-y-3">
                     <template x-if="currentSerialItem">
                        <template x-for="i in parseInt(currentSerialItem.quantity)" :key="i">
                            <div class="flex items-center gap-3 group italic font-black tracking-widest uppercase">
                ...
                <div class="mt-8 grid grid-cols-2 gap-4">
                    ...
                    <button @click="saveSerials" class="py-4 bg-primary-500 hover:bg-primary-600 text-white rounded-2xl text-[11px] font-black uppercase tracking-widest transition-all shadow-lg shadow-primary-500/40">Keep Serials</button>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
             Alpine.data('transferForm', () => ({
                form: {
                    warehouse_from: '{{ $transfer->warehouse_from }}',
                    warehouse_to: '{{ $transfer->warehouse_to }}',
                    reference_no: '{{ $transfer->reference_no }}',
                    transfer_date: '{{ $transfer->transfer_date }}',
                    note: `{!! addslashes($transfer->note) !!}`,
                    items: @json($transferItems)
                },
                searchQuery: '',
                searchResults: [],
                ...
                init() {
                    this.$watch('form.warehouse_from', (value, oldValue) => {
                    ...
                    this.form.items.forEach(item => {
                         if (!Array.isArray(item.serials)) item.serials = [];
                    });
                },
                ...
                openSerialModal(index) {
                    ...
                    const qty = parseInt(item.quantity) || 0;
                    while (this.tempSerials.length < qty) this.tempSerials.push('');
                ...
                async submitForm() {
                    ...
                    try {
                        const response = await fetch(`{{ route('stock.transfer.update', $transfer->id, false) }}`, {
                            method: 'POST',
                            headers: {
                        ...
                        if (result.success) {
                            showSuccess(result.message || 'Stock Transfer updated successfully!');
                            setTimeout(() => { window.location.href = "{{ route('stock.transfer') }}"; }, 800);
                        } else {
                            showError(result.message || 'Failed to update transfer.');
                        }
                    } catch (error) {
