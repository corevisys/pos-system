<x-app-layout title="Variants List">
    <div x-data="{
        deleteModal: false,
        deleteId: null,
        ...
        toggleAll() {
            ...
            if (this.selectedAll) {
                this.selectedVariants = Array.from(checkboxes).map(cb => cb.value);
            } else {
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
            <div>
                <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Variant Management</h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-bold uppercase tracking-widest">Variant List</span>
                </div>
            </div>

            <a href="{{ route('items.variants.add') }}" class="w-full md:w-auto px-4 py-1.5 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all shadow-lg shadow-rose-200/50 dark:shadow-none flex items-center justify-center gap-2 text-center">
        ...
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div class="bg-white dark:bg-dark-card p-3 rounded-2xl border border-slate-100 dark:border-dark-border group hover:shadow-lg transition-all duration-300">
                    ...
                    <p class="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-0.5">Total Variants</p>
            ...
            <div class="bg-white dark:bg-dark-card rounded-3xl border border-slate-100 dark:border-dark-border overflow-hidden shadow-sm">
                ...
                <!-- Table Controls -->
                <form x-ref="filterForm" action="{{ route('items.variants') }}" method="GET" class="px-4 py-3 border-b border-slate-50 dark:border-dark-border bg-slate-50/20 dark:bg-white/5 flex flex-wrap justify-between items-center gap-4">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-dark-border">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest cursor-pointer hover:text-primary-600 transition-colors">
                                    <a href="{{ request()->fullUrlWithQuery(['sort' => 'variant_name', 'order' => request('order') === 'asc' ? 'desc' : 'asc']) }}" class="flex items-center gap-1">
                                        Variant Name
                                        @if(request('sort') === 'variant_name')
                                ...
                                <th class="px-6 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Variant Code</th>
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-dark-border">
                            @forelse($variants as $variant)
                                <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                    <td class="px-6 py-2.5 text-center">
                                        <input type="checkbox" value="{{ $variant->id }}" x-model="selectedVariants" class="variant-checkbox w-3.5 h-3.5 rounded border-slate-300 text-primary-600 focus:ring-primary-500 cursor-pointer">
                                    </td>
                                    <td class="px-6 py-2.5">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="text-[11px] font-bold text-slate-700 dark:text-slate-200">{{ $variant->variant_name }}</span>
                                        </div>
                                    </td>
                                    <td class="px-6 py-2.5">
                                        <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 leading-tight uppercase tracking-widest italic">{{ $variant->variant_code ?? '---' }}</span>
                                    </td>
                                    <td class="px-6 py-2.5">
                                        <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 leading-tight">{{ $variant->description ?? 'No description' }}</span>
                                    </td>
                                    <td class="px-6 py-2.5 text-center">
                                        @if($variant->status)
                                         ...
                                         <div x-data="{ open: false }" class="relative inline-block text-left">
                                            ...
                                            <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-36 origin-top-right rounded-xl bg-white dark:bg-dark-card shadow-2xl border border-slate-100 dark:border-dark-border z-20 overflow-hidden" x-cloak>
                                                <div class="py-1 text-left">
                                                    <a href="{{ route('items.variants.edit', $variant->id) }}" class="w-full text-left block px-4 py-2 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest">Edit</a>
                                                    <button @click="openDeleteModal({{ $variant->id }}); open = false" class="w-full text-left block px-4 py-2 text-[10px] font-bold text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors uppercase tracking-widest border-t border-slate-50 dark:border-dark-border italic">Delete</button>
                                                </div>
                                            </div>
                                         </div>
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="px-6 py-10 text-center text-[10px] font-bold text-slate-400 italic uppercase tracking-widest">No variants found</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div x-show="deleteModal" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="fixed inset-0 z-[100] flex items-center justify-center bg-dark-bg/40 backdrop-blur-md p-6">
        <div x-show="deleteModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="scale-95 translate-y-10" x-transition:enter-end="scale-100 translate-y-0" @click.away="deleteModal = false"
            ...
            <h2 class="text-2xl font-black mb-2">Delete Variant?</h2>
            <p class="text-slate-400 text-sm font-medium mb-8 leading-relaxed">This will remove the variant from the system. <br> Existing items linked to this variant may be affected.</p>
            
            <form :action="'{{ url('items/variants') }}/' + deleteId" method="POST" class="flex flex-col gap-3">
                @csrf
                <input type="hidden" name="_method" value="DELETE">
                <button type="submit" class="w-full py-4 bg-rose-600 hover:bg-rose-700 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-rose-200 dark:shadow-none">Yes, Delete Variant</button>
