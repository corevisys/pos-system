<x-app-layout title="Variants List">
    <div x-data="variantsListPage()">

        <!-- HEADER & ACTIONS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">Variants <span class="text-[10px] font-bold text-text-muted ml-2 italic uppercase tracking-widest">List</span></h1>
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <span class="text-text-secondary text-[10px] font-bold uppercase tracking-wider">Variants List</span>
                </div>
            </div>

            <div class="flex flex-wrap items-center gap-2 w-full md:w-auto">
                <a href="{{ route('items.variants.add') }}" class="btn-primary w-full md:w-auto">
        ...
        <div class="card p-3 mb-4">
            <form id="variantFilterForm" x-ref="filterForm" action="{{ route('items.variants') }}" method="GET" class="flex flex-wrap justify-between items-center gap-4">
        ...
        <x-table>
            <x-slot name="thead">
                <tr>
                    <th class="px-4 py-3 text-[9px] font-black text-text-muted uppercase tracking-widest whitespace-nowrap">Variant Name</th>
                    <th class="px-4 py-3 text-[9px] font-black text-text-muted uppercase tracking-widest whitespace-nowrap">Variant Code</th>
                ...
                <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                    <td class="px-4 py-2 whitespace-nowrap">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-[11px] font-bold text-text-primary dark:text-dark-text">{{ $variant->variant_name }}</span>
                        </div>
                    </td>
                    <td class="px-4 py-2 whitespace-nowrap">
                        <span class="text-[10px] font-bold text-text-muted leading-tight uppercase tracking-widest italic">{{ $variant->variant_code ?? '---' }}</span>
                    </td>
                    <td class="px-4 py-2">
                        <span class="text-[10px] font-bold text-text-secondary leading-tight">{{ $variant->description ?? 'No description' }}</span>
                    </td>
                    ...
                    <td class="px-4 py-2 text-center whitespace-nowrap">
                        <x-dropdown align="right" width="40">
                            ...
                            <x-slot name="content">
                                <x-dropdown-link :href="route('items.variants.edit', $variant->id)">Edit</x-dropdown-link>
                                <button type="button" @click="toggleVariantStatus({{ $variant->id }}, {{ $variant->status ? '0' : '1' }})" class="w-full text-left px-3 py-1.5 text-[9px] font-black uppercase tracking-widest {{ $variant->status ? 'text-amber-600' : 'text-emerald-600' }} hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors">
                                    {{ $variant->status ? 'Deactivate' : 'Activate' }}
                                </button>
                                <button type="button" @click="openDeleteModal({{ $variant->id }}, '{{ addslashes($variant->variant_name) }}')" class="w-full text-left px-3 py-1.5 text-[9px] font-black uppercase tracking-widest text-danger hover:bg-rose-50 dark:hover:bg-rose-500/10 transition-colors border-t border-border dark:border-dark-border">
                                    Delete
                                </button>
                            </x-slot>
                        </x-dropdown>
                ...
                <tr>
                    <td colspan="5" class="px-4 py-10 text-center">
                        <div class="flex flex-col items-center gap-2 opacity-40">
                            <p class="text-[10px] font-black uppercase tracking-widest text-text-muted">No Variants Found!!</p>
                        </div>
                    </td>
                </tr>
            @endforelse
        ...
        <div x-show="deleteModal" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" class="fixed inset-0 z-[100] flex items-center justify-center bg-dark-bg/40 backdrop-blur-md p-6">
            <div x-show="deleteModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="scale-95 translate-y-10" x-transition:enter-end="scale-100 translate-y-0" @click.away="deleteModal = false"
                ...
                <p class="text-slate-400 text-sm font-medium mb-8 leading-relaxed">
                    This will remove the variant from the system.<br>
                    Existing items linked to this variant may be affected.
                </p>
    ...
    document.addEventListener('alpine:init', () => {
        Alpine.data('variantsListPage', () => ({
            ...
            toggleVariantStatus(id, newStatus) {
                fetch('/items/variants/toggle-status/' + id, {
                    method: 'PATCH',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        if (window.showSuccess) window.showSuccess(data.message || 'Variant updated.');
