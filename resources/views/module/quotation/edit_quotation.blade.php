<x-app-layout title="Edit Quotation">
    <div x-data="{
        quotationInfo: {
            ...
            quotationDate: '{{ \Carbon\Carbon::parse($quotation->quotation_date)->format('Y-m-d') }}',
            expireDate: '{{ \Carbon\Carbon::parse($quotation->expire_date)->format('Y-m-d') }}',
            referenceNo: '{{ $quotation->reference_no }}',
            note: '{{ addslashes($quotation->quotation_note ?? '') }}',
        ...
        cart: [
            @foreach($quotation->items as $item)
            {
                ...
                name: '{{ addslashes($item->item->item_name ?? 'Item') }}',
                qty: {{ (float) ($item->quotation_qty ?? 0) }},
                price: {{ (float) ($item->price_per_unit ?? 0) }},
                discount: {{ (float) ($item->discount_amt ?? 0) }},
                tax: {{ (float) ($item->item->tax->tax ?? 0) }},
                tax_type: '{{ $item->tax_type ?? ($item->item->tax_type ?? 'Inclusive') }}',
                taxAmount: {{ (float) ($item->tax_amt ?? 0) }},
                unitCost: {{ (float) ($item->unit_total_cost ?? 0) }},
        ...
        searchResults: [],
        searchQuery: '',
        otherCharges: {{ (float) ($quotation->other_charges_input ?? 0) }},
        otherChargesTaxAmount: {{ (float) ($quotation->other_charges_amt ?? 0) }},
        discountOnAll: {{ (float) ($quotation->discount_to_all_input ?? 0) }},
        discountType: '{{ $quotation->discount_to_all_type ?? 'Fixed' }}',
        ...
        async updateQuotation() {
            ...
            try {
                ...
                const response = await fetch('{{ route('quotation.update', $quotation->id, false) }}', {
                    method: 'POST',
                    headers: { 
                ...
                if (response.ok && result.success) {
                    this.successMessage = result.message || 'Quotation updated successfully!';
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">Edit Quotation</h1>
                <div class="flex items-center gap-2 text-text-secondary dark:text-dark-text/60 font-medium mt-0.5 text-xs">
                    ...
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-text-primary dark:text-dark-text font-mono font-bold">{{ $quotation->quotation_code }}</span>
                </div>
            </div>

            <div class="flex items-center gap-2">
                <a href="{{ route('quotation.invoice', $quotation->id) }}" class="btn-secondary px-3.5 py-1.5 text-xs font-bold flex items-center gap-1">
            ...
            <div class="card p-4 space-y-3 bg-slate-900 text-white dark:bg-dark-card border-none shadow-md">
                ...
                <div class="pt-2">
                    <button 
                        ...
                        <span x-text="isSubmitting ? 'Updating Quotation...' : 'Update Quotation'"></span>
                    </button>
                </div>
            </div>
        </div>
