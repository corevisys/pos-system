<x-app-layout title="Add State">
    <div class="max-w-4xl mx-auto" x-data="{ isSubmitting: false }">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-8">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">Add State <span class="text-[10px] font-bold text-text-muted ml-2 italic uppercase tracking-widest">Create New Record</span></h1>
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-text-secondary text-[10px] font-black uppercase tracking-wider">Add State</span>
                </div>
            </div>
        </div>

        <!-- FORM CARD -->
        <form action="{{ route('settings.states.store') }}" method="POST" @submit="isSubmitting = true">
            @csrf
            <x-card padding="p-0" class="overflow-hidden">
                <div class="p-8 border-b border-border dark:border-dark-border">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="group relative">
                            ...
                            <input type="text" name="state" id="state" value="{{ old('state') }}" required class="input-base !py-3 !text-sm !font-bold" placeholder="e.g. Dhaka">
                ...
                <div class="p-6 bg-background/50 dark:bg-white/5 flex flex-col sm:flex-row justify-end items-center gap-3">
                    ...
                    <button type="submit" :disabled="isSubmitting" class="btn-primary w-full sm:w-auto px-10 disabled:opacity-60 disabled:cursor-not-allowed">Save State</button>
                </div>
            </x-card>
        </form>
    </div>
