<x-app-layout title="Add Country">
    <div class="max-w-4xl mx-auto" x-data="{ isSubmitting: false }">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-8">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">Add Country <span class="text-[10px] font-bold text-text-muted ml-2 italic uppercase tracking-widest">Create New Record</span></h1>
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-text-secondary text-[10px] font-black uppercase tracking-wider">Add Country</span>
                </div>
            </div>
        </div>

        <!-- FORM CARD -->
        <form action="{{ route('settings.countries.store') }}" method="POST" @submit="isSubmitting = true">
            @csrf
            <x-card padding="p-0" class="overflow-hidden">
                <div class="p-8 border-b border-border dark:border-dark-border">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="group relative">
                            ...
                            <input type="text" name="country" id="country" value="{{ old('country') }}" required class="input-base !py-3 !text-sm !font-bold" placeholder="e.g. Bangladesh">
                ...
                <div class="p-6 bg-background/50 dark:bg-white/5 flex flex-col sm:flex-row justify-end items-center gap-3">
                    ...
                    <button type="submit" :disabled="isSubmitting" class="btn-primary w-full sm:w-auto px-10 disabled:opacity-60 disabled:cursor-not-allowed">Save Country</button>
                </div>
            </x-card>
        </form>
    </div>
