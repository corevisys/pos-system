<x-app-layout title="Edit State">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-8">
            <div>
                <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Edit State <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Modify Details</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Edit State</span>
                </div>
            </div>
        </div>

        <!-- FORM CARD -->
        <form action="{{ route('settings.states.update', $state->id) }}" method="POST" class="bg-white dark:bg-dark-card rounded-3xl border border-slate-100 dark:border-dark-border shadow-2xl shadow-slate-200/50 dark:shadow-none overflow-hidden transition-all duration-300">
            @csrf
            ...
            <div class="p-8 border-b border-slate-50 dark:border-dark-border">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="relative group">
                        ...
                        <input type="text" name="state" id="state" value="{{ old('state', $state->state) }}" required
