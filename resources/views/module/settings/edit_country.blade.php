<x-app-layout title="Edit Country">
    <div class="max-w-4xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-8">
            <div>
                <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Edit Country <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">Modify Details</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <a href="{{ route('settings.countries') }}" class="hover:text-primary-600 transition-colors text-[10px] font-bold uppercase tracking-wider">Countries List</a>
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Edit Country</span>
                </div>
            </div>
        </div>

        <!-- FORM CARD -->
        <form action="{{ route('settings.countries.update', $country->id) }}" method="POST" class="bg-white dark:bg-dark-card rounded-3xl border border-slate-100 dark:border-dark-border shadow-2xl shadow-slate-200/50 dark:shadow-none overflow-hidden transition-all duration-300">
            @csrf
            ...
            <div class="p-8 border-b border-slate-50 dark:border-dark-border">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="relative group">
                        <label for="country" class="absolute -top-2 left-4 px-2 bg-white dark:bg-dark-card text-[10px] font-black uppercase tracking-widest text-slate-400 group-focus-within:text-primary-600 transition-all z-10">Country Name <span class="text-rose-500">*</span></label>
                        <input type="text" name="country" id="country" value="{{ old('country', $country->country) }}" required
                        ...
                        @error('country') <p class="text-[10px] text-rose-500 font-bold mt-1 ml-2 uppercase italic tracking-widest">{{ $message }}</p> @enderror
                    </div>
            ...
            <div class="p-6 bg-slate-50/50 dark:bg-white/5 flex flex-col sm:flex-row justify-end items-center gap-3">
                <a href="{{ route('settings.countries') }}" class="w-full sm:w-auto px-8 py-3 bg-white dark:bg-dark-card border-2 border-slate-100 dark:border-dark-border rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 hover:text-slate-600 hover:border-slate-200 transition-all text-center">
                    Cancel
                </a>
