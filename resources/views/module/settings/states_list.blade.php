<x-app-layout title="States List">
    <div>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">States List <span class="text-[10px] font-bold text-text-muted ml-2 italic uppercase tracking-widest">View/Search States</span></h1>
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <span class="text-text-secondary text-[10px] font-bold uppercase tracking-widest">States List</span>
                </div>
            </div>

            <a href="{{ route('settings.states.add') }}" class="btn-primary w-full md:w-auto !bg-danger hover:!bg-danger-hover">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            <div class="card p-4">
                <p class="text-[8px] font-black text-text-muted uppercase tracking-widest mb-1">Total States</p>
                <h3 class="text-2xl font-black text-text-primary dark:text-white tabular-nums">{{ $totalStates }}</h3>
            </div>
            <div class="card p-4">
                <p class="text-[8px] font-black text-text-muted uppercase tracking-widest mb-1 text-success">Active</p>
                <h3 class="text-2xl font-black text-success tabular-nums">{{ $activeStates }}</h3>
            </div>
            <div class="card p-4">
                <p class="text-[8px] font-black text-text-muted uppercase tracking-widest mb-1 text-danger">Inactive</p>
                <h3 class="text-2xl font-black text-danger tabular-nums">{{ $inactiveStates }}</h3>
            </div>
        </div>

        <!-- SEARCH BAR -->
        <form action="{{ route('settings.states') }}" method="GET" class="card p-3 mb-4 flex flex-wrap justify-between items-center gap-3">
            <div class="relative group">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search states or country..." class="input-base !w-56 !py-1.5 !pl-8 !pr-3 !text-[10px] !rounded-xl shadow-sm">
            ...
            <div class="flex items-center gap-2">
                ...
                @if(request('search'))
                    <a href="{{ route('settings.states') }}" class="text-[9px] font-black uppercase tracking-wider text-danger hover:underline">Clear</a>
                @endif
            </div>
        </form>
                ...
                <table class="w-full text-left">
                    <thead class="bg-background dark:bg-dark-bg border-b border-border dark:border-dark-border">
                        <tr>
                            ...
                            <th class="px-4 py-2.5 text-[9px] font-black text-text-muted uppercase tracking-widest">State Name</th>
                            <th class="px-4 py-2.5 text-[9px] font-black text-text-muted uppercase tracking-widest">Country</th>
                        ...
                        @forelse($states as $index => $state)
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                <td class="px-4 py-1.5 text-center text-[10px] font-bold text-text-muted tabular-nums">
                                    {{ $states->firstItem() + $index }}
                                </td>
                                <td class="px-4 py-1.5">
                                    <span class="text-[11px] font-bold text-text-primary dark:text-dark-text tracking-tight transition-transform group-hover:translate-x-1 inline-block">{{ $state->state }}</span>
                                </td>
                                <td class="px-4 py-1.5">
                                    <div class="flex items-center gap-2">
                                        <div class="w-6 h-4 bg-background dark:bg-slate-800 rounded flex items-center justify-center text-[8px] font-black text-text-muted uppercase tracking-widest border border-border-light dark:border-dark-border">
                                            {{ substr($state->country, 0, 2) }}
                                        </div>
                                        <span class="text-[10px] font-bold text-text-secondary uppercase tracking-wider">{{ $state->country }}</span>
                                    </div>
                                </td>
                                <td class="px-4 py-1.5 text-center">
                                    <span class="{{ $state->status == 1 ? 'bg-success/10 text-success border-success/20' : 'bg-danger/10 text-danger border-danger/20' }} px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest border">
                                ...
                                <td class="px-4 py-1.5 text-center">
                                    <x-dropdown align="right" width="40">
                                        ...
                                        <x-slot name="content">
                                            <a href="{{ route('settings.states.edit', $state->id) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-text-primary dark:text-dark-text hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors uppercase tracking-widest">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-8 text-center">
                                    <p class="text-[9px] font-black text-text-muted uppercase tracking-widest italic leading-none">No states found.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
