<x-app-layout title="Countries List">
    <div>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Countries List <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">View/Search Countries</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Countries List</span>
                </div>
            </div>

            <div class="flex items-center gap-2">
                <a href="{{ route('settings.countries.add') }}" class="px-4 py-2 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all shadow-lg shadow-rose-200/50 dark:shadow-none flex items-center justify-center gap-2">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
            <div class="bg-white dark:bg-dark-card p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-dark-border group hover:border-primary-500 transition-all duration-300">
                <p class="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Total Countries</p>
                <h3 class="text-2xl font-black text-slate-800 dark:text-white tabular-nums">{{ $totalCountries }}</h3>
            </div>
            <div class="bg-white dark:bg-dark-card p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-dark-border group hover:border-emerald-500 transition-all duration-300">
                ...
                <h3 class="text-2xl font-black text-slate-800 dark:text-white tabular-nums">{{ $activeCountries }}</h3>
            </div>
            <div class="bg-white dark:bg-dark-card p-4 rounded-2xl shadow-sm border border-slate-100 dark:border-dark-border group hover:border-rose-500 transition-all duration-300">
                ...
                <h3 class="text-2xl font-black text-slate-800 dark:text-white tabular-nums">{{ $inactiveCountries }}</h3>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div class="bg-white dark:bg-dark-card rounded-2xl border border-slate-100 dark:border-dark-border overflow-hidden shadow-sm">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-dark-border">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest tracking-[0.2em]">Country Name</th>
                        ...
                        <tbody class="divide-y divide-slate-50 dark:divide-dark-border">
                            @forelse($countries as $index => $country)
                                <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                    ...
                                    <td class="px-4 py-2.5">
                                        <span class="text-[11px] font-bold text-slate-700 dark:text-slate-200 tracking-tight transition-transform group-hover:translate-x-1 inline-block">{{ $country->country }}</span>
                                    </td>
                                    ...
                                    <td class="px-4 py-2.5 text-center">
                                        <div x-data="{ open: false }" class="relative inline-block text-left">
                                            ...
                                            <div x-show="open" 
                                                ...
                                                <div class="py-1">
                                                    <a href="{{ route('settings.countries.edit', $country->id) }}" class="flex items-center gap-2 px-3 py-2 text-[9px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest group/item">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-8 text-center text-slate-400 text-[10px] uppercase font-black tracking-widest">No countries found.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="p-3 border-t border-slate-50 dark:border-dark-border bg-slate-50/20 dark:bg-white/5 flex flex-wrap justify-between items-center gap-4">
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic tracking-[0.2em]">Showing {{ $countries->count() }} entries</p>
