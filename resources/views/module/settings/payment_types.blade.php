<x-app-layout title="Payment Types">
    <div x-data="{
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Payment Types <span class="text-[10px] font-bold text-slate-400 ml-2 italic uppercase tracking-widest">View/Search Payment Types</span></h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Payment Types</span>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 gap-6">
            ...
            <div class="bg-white dark:bg-dark-card rounded-2xl border border-slate-100 dark:border-dark-border overflow-hidden shadow-sm">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead class="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-dark-border">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-[9px] font-black text-slate-400 uppercase tracking-widest">Payment Type Name</th>
                            ...
                            @forelse ($paymentTypes as $p)
                                <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                    ...
                                    <td class="px-4 py-2.5">
                                        <span class="text-[11px] font-bold text-slate-700 dark:text-slate-200 uppercase tracking-tight">{{ $p->payment_type }}</span>
                                    </td>
                                    <td class="px-4 py-2.5 text-center">
                                        @if ($p->status)
                                    ...
                                    <td class="px-4 py-2.5 text-center">
                                        @if ($p->payment_type != 'CASH')
                                            <div x-data="{ open: false }" class="relative inline-block text-left">
                                                ...
                                                <div x-show="open" @click.away="open = false" class="absolute right-0 mt-2 w-28 origin-top-right rounded-xl bg-white dark:bg-dark-card shadow-xl border border-slate-100 dark:border-dark-border z-20 overflow-hidden" x-cloak>
                                                    <div class="py-1">
                                                        <button @click="editId = {{ $p->id }}; editName = '{{ $p->payment_type }}'; editStatus = {{ $p->status }}; showEditModal = true; open = false;" class="w-full text-left block px-3 py-2 text-[9px] font-bold text-slate-600 hover:bg-slate-50 dark:hover:bg-slate-500/10 transition-colors uppercase tracking-widest">Edit</button>
                                                        <form action="{{ route('settings.payment_types.delete', $p->id) }}" method="POST" onsubmit="return confirm('Are you sure?')">
                                                            @csrf
                                                            @method('DELETE')
                                        ...
                                        @else
                                            <span class="text-[9px] font-black text-slate-300 uppercase tracking-widest italic">-NA-</span>
                                        @endif
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="px-4 py-8 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">No Payment Types Found</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="p-3 border-t border-slate-50 dark:border-dark-border bg-slate-50/20 dark:bg-white/5 flex flex-wrap justify-between items-center gap-4">
                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest italic">Showing {{ $paymentTypes->count() }} entries</p>
                </div>
            </div>
        </div>
                ...
                <div x-show="showAddModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" class="inline-block align-bottom bg-white dark:bg-dark-card rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-slate-100 dark:border-dark-border">
                    <form action="{{ route('settings.payment_types.store') }}" method="POST">
                        ...
                        <div class="px-6 py-4 border-b border-slate-50 dark:border-dark-border flex justify-between items-center">
                            <h3 class="text-xs font-black uppercase tracking-[0.2em] text-slate-800 dark:text-white">Add Payment Type</h3>
                        ...
                        <div class="px-6 py-6 space-y-4">
                            <div class="space-y-1">
                                <label class="text-[9px] font-black uppercase tracking-widest text-slate-400">Payment Type Name</label>
                                <input type="text" name="payment_type" required placeholder="e.g. Bkash" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-dark-border rounded-xl text-xs font-bold text-slate-700 dark:text-white focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all">
                            </div>
                            <div class="space-y-1">
                        ...
                        <div class="px-6 py-4 bg-slate-50/50 dark:bg-white/5 border-t border-slate-50 dark:border-dark-border flex justify-end gap-3">
                            ...
                            <button type="submit" class="px-6 py-2 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all shadow-lg shadow-rose-200/50 dark:shadow-none">Save Payment Type</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showEditModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" class="inline-block align-bottom bg-white dark:bg-dark-card rounded-2xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full border border-slate-100 dark:border-dark-border">
                    <form :action="'/settings/payment-types/' + editId" method="POST">
                        @csrf
                        <div class="px-6 py-4 border-b border-slate-50 dark:border-dark-border flex justify-between items-center">
                            <h3 class="text-xs font-black uppercase tracking-[0.2em] text-slate-800 dark:text-white">Edit Payment Type</h3>
                        ...
                        <div class="px-6 py-6 space-y-4">
                            <div class="space-y-1">
                                <label class="text-[9px] font-black uppercase tracking-widest text-slate-400">Payment Type Name</label>
                                <input type="text" name="payment_type" required x-model="editName" class="w-full px-4 py-2.5 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-dark-border rounded-xl text-xs font-bold text-slate-700 dark:text-white focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500 outline-none transition-all">
                            </div>
                            <div class="space-y-1">
                                <label class="text-[9px] font-black uppercase tracking-widest text-slate-400">Status</label>
                        ...
                        <div class="px-6 py-4 bg-slate-50/50 dark:bg-white/5 border-t border-slate-50 dark:border-dark-border flex justify-end gap-3">
                            ...
                            <button type="submit" class="px-6 py-2 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all shadow-lg shadow-rose-200/50 dark:shadow-none">Update Payment Type</button>
                        </div>
                    </form>
                </div>
            </div>
