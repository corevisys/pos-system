<x-app-layout title="Add Language">
    <div x-data="{ isSubmitting: false }">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">Add New Language</h1>
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-text-secondary text-[10px] font-black uppercase tracking-wider">Add Language</span>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-xl mx-auto">
            <x-card padding="p-8">
                <form action="{{ route('settings.languages.store') }}" method="POST" class="space-y-8" @submit="isSubmitting = true">
                    @csrf
                    ...
                    <div class="space-y-6">
                        <div class="group relative">
                            ...
                            <input type="text" name="language" value="{{ old('language') }}" class="input-base" placeholder="e.g. English" required>
                            @error('language')
