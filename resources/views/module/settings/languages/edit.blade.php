<x-app-layout title="Edit Language">
    <div>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Edit Language</h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-slate-600 text-[10px] font-black uppercase tracking-wider">Edit Language</span>
                </div>
            </div>
        </div>
        ...
        <div class="max-w-xl mx-auto">
            <div class="bg-white dark:bg-dark-card rounded-3xl border border-slate-100 dark:border-dark-border shadow-sm overflow-hidden p-8">
                <form action="{{ route('settings.languages.update', $language->id) }}" method="POST" class="space-y-8">
                    @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="group relative">
                            ...
                            @if($language->status == 1)
                                ...
                                <div class="w-full bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-200 dark:border-emerald-800/40 rounded-2xl py-3 px-4 text-[11px] font-black text-emerald-700 dark:text-emerald-300 flex items-center justify-between">
                                ...
                                <p class="text-[9px] font-medium text-slate-400 italic mt-1.5 ml-1">To change the active language, activate another language from the languages list.</p>
                            @else
                                ...
                                <p class="text-[9px] font-medium text-slate-400 italic mt-1.5 ml-1">Changing to Active will automatically deactivate the currently active language.</p>
                            @endif
                        </div>
                    </div>
