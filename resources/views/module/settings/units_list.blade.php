<x-app-layout title="Units List">
    <div x-data="{
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-4">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">Units List</h1>
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <span class="text-text-secondary text-[10px] font-bold uppercase tracking-widest">Units List</span>
                </div>
                <p class="text-[10px] text-text-muted font-bold uppercase tracking-widest mt-1 px-1">View/Search Units</p>
            </div>
        ...
        <x-card padding="p-0" class="overflow-hidden">
            <!-- Table Controls (server-side search) -->
            <div class="px-4 py-3 border-b border-border dark:border-dark-border bg-background/40 dark:bg-white/5 flex flex-wrap justify-between items-center gap-3">
                <form action="{{ route('settings.units') }}" method="GET" class="flex flex-wrap items-center gap-3">
                    ...
                    @if(request('search'))
                        <a href="{{ route('settings.units') }}" class="text-[9px] font-black uppercase tracking-wider text-danger hover:underline">Clear</a>
                    @endif
                </form>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-left">
                    <thead class="bg-background dark:bg-dark-bg border-b border-border dark:border-dark-border">
                        <tr>
                            <th class="px-4 py-2.5 text-[9px] font-black text-text-muted uppercase tracking-widest">Unit Name</th>
                        ...
                        @forelse ($units as $unit)
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                <td class="px-4 py-1.5">
                                    <span class="text-[11px] font-bold text-text-primary dark:text-dark-text">{{ $unit->unit_name }}</span>
                                </td>
                                <td class="px-4 py-1.5">
                                    <span class="text-[10px] font-medium text-text-muted italic">{{ $unit->description ?? '---' }}</span>
                                </td>
                                <td class="px-4 py-1.5 text-center">
                                    <span class="{{ $unit->status ? 'bg-success/10 text-success border-success/20' : 'bg-danger/10 text-danger border-danger/20' }} px-2 py-0.5 rounded text-[9px] font-bold uppercase tracking-widest border">
                                ...
                                <td class="px-4 py-1.5 text-center">
                                    <x-dropdown align="right" width="40">
                                        ...
                                        <x-slot name="content">
                                            <button
                                                @click="editId = @js($unit->id); editName = @js($unit->unit_name); editDescription = @js($unit->description ?? ''); editStatus = @js((int) $unit->status); showEditModal = true"
                                            ...
                                            <form :action="'/settings/units/' + @js($unit->id)" method="POST" onsubmit="return confirm('Are you sure?')">
                                                @csrf
                                                @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-8 text-center">
                                    <p class="text-[9px] font-black text-text-muted uppercase tracking-widest italic leading-none">No units found</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                ...
                <div x-show="showAddModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" @click.stop class="relative z-10 inline-block align-bottom card rounded-2xl text-left overflow-hidden shadow-modal transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form action="/settings/units" method="POST" @submit="isSubmitting = true">
                        ...
                        <div class="px-6 py-4 border-b border-border dark:border-dark-border flex justify-between items-center">
                            <h3 class="text-xs font-black uppercase tracking-[0.2em] text-text-primary dark:text-dark-text">Add Unit</h3>
                        ...
                        <div class="px-6 py-6 space-y-4">
                            <div class="space-y-1">
                                <label class="text-[9px] font-black uppercase tracking-widest text-text-muted">Unit Name <span class="text-danger">*</span></label>
                                <input type="text" name="unit_name" required placeholder="e.g. KG" class="input-base">
                            </div>
                            <div class="space-y-1">
                                <label class="text-[9px] font-black uppercase tracking-widest text-text-muted">Description</label>
                                <textarea name="description" rows="3" placeholder="Enter description (optional)" class="input-base"></textarea>
                            </div>
                            <div class="space-y-1">
                        ...
                        <div class="px-6 py-4 bg-background/50 dark:bg-white/5 border-t border-border dark:border-dark-border flex justify-end gap-3">
                            ...
                            <button type="submit" :disabled="isSubmitting" class="btn-primary disabled:opacity-60 disabled:cursor-not-allowed">Save Unit</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="showEditModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" @click.stop class="relative z-10 inline-block align-bottom card rounded-2xl text-left overflow-hidden shadow-modal transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                    <form :action="'/settings/units/' + editId" method="POST" @submit="isSubmitting = true">
                        @csrf
                        <div class="px-6 py-4 border-b border-border dark:border-dark-border flex justify-between items-center">
                            <h3 class="text-xs font-black uppercase tracking-[0.2em] text-text-primary dark:text-dark-text">Edit Unit</h3>
                        ...
                        <div class="px-6 py-6 space-y-4">
                            <div class="space-y-1">
                                <label class="text-[9px] font-black uppercase tracking-widest text-text-muted">Unit Name <span class="text-danger">*</span></label>
                                <input type="text" name="unit_name" required x-model="editName" class="input-base">
                            </div>
                            <div class="space-y-1">
                                <label class="text-[9px] font-black uppercase tracking-widest text-text-muted">Description</label>
                                <textarea name="description" rows="3" x-model="editDescription" class="input-base"></textarea>
                            </div>
                            <div class="space-y-1">
                                <label class="text-[9px] font-black uppercase tracking-widest text-text-muted">Status</label>
                        ...
                        <div class="px-6 py-4 bg-background/50 dark:bg-white/5 border-t border-border dark:border-dark-border flex justify-end gap-3">
                            ...
                            <button type="submit" :disabled="isSubmitting" class="btn-primary disabled:opacity-60 disabled:cursor-not-allowed">Update Unit</button>
                        </div>
                    </form>
                </div>
            </div>
