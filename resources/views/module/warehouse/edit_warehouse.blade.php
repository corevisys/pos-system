<x-app-layout title="Edit Warehouse">
    <div class="flex flex-col items-center">
        ...
        <div class="w-full max-w-2xl mb-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
            <div>
                <h1 class="text-xl font-black tracking-tight text-slate-800 dark:text-white">Edit Warehouse</h1>
                <div class="flex items-center gap-2 text-slate-400 font-medium mt-1">
                    ...
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-slate-600 text-[10px] font-bold uppercase tracking-widest">Edit Warehouse</span>
                </div>
            </div>
        </div>
        ...
        <div class="w-full max-w-2xl bg-white dark:bg-dark-card rounded-3xl border border-slate-100 dark:border-dark-border shadow-sm overflow-hidden p-6 md:p-10">
            <form action="{{ route('warehouse.update', $warehouse->id) }}" method="POST" class="space-y-8">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div class="group relative">
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="warehouse_name" value="{{ old('warehouse_name', $warehouse->warehouse_name) }}" placeholder="Enter warehouse name..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl py-3 pl-10 pr-4 text-[11px] font-bold text-slate-700 dark:text-slate-300 outline-none transition-all focus:ring-1 focus:ring-primary-500" required>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="group relative">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="mobile" value="{{ old('mobile', $warehouse->mobile) }}" placeholder="Mobile..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl py-3 pl-10 pr-4 text-[11px] font-bold text-slate-700 dark:text-slate-300 outline-none transition-all focus:ring-1 focus:ring-emerald-500">
                            </div>
                            @error('mobile')
                                <p class="text-rose-500 text-[9px] font-bold mt-1 px-4 italic uppercase tracking-widest">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="group relative">
                            ...
                            <div class="relative">
                                ...
                                <input type="email" name="email" value="{{ old('email', $warehouse->email) }}" placeholder="Email..." class="w-full bg-slate-50 dark:bg-slate-800 border-none rounded-xl py-3 pl-10 pr-4 text-[11px] font-bold text-slate-700 dark:text-slate-300 outline-none transition-all focus:ring-1 focus:ring-emerald-500">
                            </div>
                            @error('email')
                                <p class="text-rose-500 text-[9px] font-bold mt-1 px-4 italic uppercase tracking-widest">{{ $message }}</p>
                            @enderror
                    ...
                    <!-- Status Toggle -->
                    <div class="group relative">
                        <label class="absolute -top-2 left-6 bg-white dark:bg-dark-card px-2 text-[8px] font-black uppercase text-slate-400 tracking-widest z-10 italic">
                            Status
                        </label>
                        <div class="flex items-center gap-4 bg-slate-50 dark:bg-slate-800 rounded-xl p-3">
                            <label class="flex items-center cursor-pointer gap-3">
                                ...
                                <span class="text-[10px] font-black uppercase tracking-widest text-slate-500 peer-checked:text-primary-600">Active</span>
                            </label>
                        </div>
                    </div>
                </div>
