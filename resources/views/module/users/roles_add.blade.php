<x-app-layout title="Create Role">
    <div>
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">Define New Role</h1>
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-text-secondary text-[10px] font-black uppercase tracking-wider">New Role</span>
                </div>
            </div>
        ...
        <x-card class="relative overflow-hidden"
             x-data="{ 
                ...
                allPermissions: [],
                init() {
                    // Populate allPermissions for Grant All button
            ...
            <form action="{{ route('users.roles.store') }}" method="POST" class="space-y-6 relative z-10">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
                    <div class="group relative">
                        ...
                        <input type="text" name="role_name" value="{{ old('role_name', 'Cashier') }}" required placeholder="e.g. Area Manager" class="input-base">
                    </div>
                    ...
                    <div class="group relative">
                        ...
                        <textarea name="description" rows="1" placeholder="Define core responsibilities..." class="input-base min-h-[38px]">{{ old('description') }}</textarea>
                    </div>
                </div>
                ...
                <div class="pt-2">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            <h4 class="text-[10px] font-black uppercase tracking-widest text-text-secondary dark:text-dark-text italic">Assign Privileges</h4>
                        </div>
                        <div class="flex items-center gap-3">
                             <button type="button" @click="selectedPermissions = [...allPermissions]" class="btn-ghost px-2 py-1 text-[8px] font-black uppercase tracking-widest">Grant All</button>
                            ...
                            <tbody class="divide-y divide-border-light dark:divide-dark-border">
                                @php
                                    $modules = [
                                        ...
                                        ['id' => 23, 'name' => 'Sales (Include POS)', 'perms' => ['Add', 'Edit', 'Delete', 'View', 'Sales Payments View', 'Sales Payments Add', 'Sales Payments Delete', 'Show all users Sales Invoices', 'Show Item Purchase Price(While making invoice)'], 'perms_map' => ['Add' => 'sales_add', 'Edit' => 'sales_edit', 'Delete' => 'sales_delete', 'View' => 'sales_view', 'Sales Payments View' => 'sales_payment_view', 'Sales Payments Add' => 'sales_payment_add', 'Sales Payments Delete' => 'sales_payment_delete', 'Show all users Sales Invoices' => 'show_all_users_sales_invoices', 'Show Item Purchase Price(While making invoice)' => 'show_purchase_price']],
                                        ...
                                        ['id' => 30, 'name' => 'Reports', 'perms' => ['View Reports'], 'perms_map' => ['View Reports' => 'reports_view']],
                ...
                <div class="flex flex-col md:flex-row justify-between items-center gap-4 pt-6 border-t border-border dark:border-dark-border">
                    <div class="flex items-center gap-2 text-text-muted">
                         ...
                         <p class="text-[9px] font-bold italic uppercase tracking-wider">New roles are <span class="text-danger">Active</span> by default.</p>
                    </div>
                    <div class="flex gap-2 w-full md:w-auto">
                        <button type="submit" class="btn-primary flex-1 md:flex-none px-10">
