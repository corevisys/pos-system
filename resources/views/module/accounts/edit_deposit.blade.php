<x-app-layout title="Edit Deposit">
    <div>
        <!-- HEADER & BREADCRUMBS -->
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-6">
            <div>
                ...
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <a href="{{ route('accounts.deposit') }}" class="hover:text-primary transition-colors text-[10px]">Deposit List</a>
                    <svg class="w-2.5 h-2.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 5l7 7-7 7"></path></svg>
                    <span class="text-text-secondary text-[10px] font-bold uppercase tracking-widest">Edit Deposit #{{ $deposit->id }}</span>
                </div>
            </div>

            <a href="{{ route('accounts.deposit') }}" class="btn-secondary w-full md:w-auto">
        ...
        <form action="{{ route('accounts.deposit.update', $deposit->id) }}" method="POST" class="max-w-4xl mx-auto" x-data="{ isSubmitting: false }" @submit="if(isSubmitting) { $event.preventDefault(); return false; } isSubmitting = true;">
            @csrf
            @method('PUT')
                ...
                <div class="px-8 py-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                        ...
                        <div class="group relative">
                            <label class="absolute -top-2 left-4 bg-card dark:bg-dark-card px-2 text-[8px] font-black uppercase text-text-muted tracking-widest z-10 transition-colors group-focus-within:text-primary">Deposit Date <span class="text-danger">*</span></label>
                            <div class="relative">
                                ...
                                <input type="date" name="deposit_date" value="{{ old('deposit_date', $deposit->deposit_date) }}" required class="input-base !pl-12 !py-3.5 !text-[11px] !font-bold">
                            </div>
                        </div>
                        ...
                        <div class="group relative">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="reference_no" value="{{ old('reference_no', $deposit->reference_no) }}" placeholder="Optional reference" class="input-base !pl-12 !py-3.5 !text-[11px] !font-bold">
                            </div>
                        </div>

                        <!-- Debit Account (Source - Optional) -->
                        <div class="group relative">
                            <label class="absolute -top-2 left-4 bg-card dark:bg-dark-card px-2 text-[8px] font-black uppercase text-text-muted tracking-widest z-10 transition-colors group-focus-within:text-danger">Debit Account (Source - Optional)</label>
                            <div class="relative">
                                ...
                                <div class="pl-4">
                                    <x-searchable-select name="debit_account_id" :options="$accounts" labelKey="account_name" valueKey="id" emptyOption="External Deposit (No Debit Account)" emptyValue="" placeholder="External Deposit (No Debit Account)" :value="old('debit_account_id', $deposit->debit_account_id)" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="group relative">
                            <label class="absolute -top-2 left-4 bg-card dark:bg-dark-card px-2 text-[8px] font-black uppercase text-text-muted tracking-widest z-10 transition-colors group-focus-within:text-success">Credit Account (Destination) <span class="text-danger">*</span></label>
                            <div class="relative">
                                <span class="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted group-focus-within:text-success transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M3 10h18M7 15h1m4 0h1m-7 4h12a2 2 0 002-2V5a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                </span>
                                <div class="pl-4">
                                    <x-searchable-select name="credit_account_id" :options="$accounts" labelKey="account_name" valueKey="id" emptyOption="Select Destination Account" emptyValue="" placeholder="Select Destination Account" :value="old('credit_account_id', $deposit->credit_account_id)" required />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="group relative md:col-span-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="number" step="0.01" name="amount" value="{{ old('amount', $deposit->amount) }}" required class="input-base !pl-14 !py-5 !text-2xl !font-black tabular-nums">
                        ...
                        <div class="group relative md:col-span-2">
                            ...
                            <div class="relative">
                                ...
                                <textarea name="note" rows="2" placeholder="Add a note..." class="input-base !pl-12 !py-3.5 !text-[11px] !font-bold resize-none">{{ old('note', $deposit->note) }}</textarea>
                            </div>
                        </div>
                    ...
                    <div class="mt-8 p-4 rounded-card bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800/30 flex items-start gap-3">
                        ...
                        <div class="text-[10px] text-amber-800 dark:text-amber-300">
                            <span class="font-bold">Ledger Integrity Notice:</span> Modifying the source, destination, or amount will automatically reverse the previous ledger impact with auditable <span class="font-mono font-bold">DEPOSIT REVERSAL</span> entries and write new forward entries.
                        </div>
                    </div>
                    ...
                    <div class="mt-10 flex flex-col md:flex-row gap-4">
                        ...
                        <a href="{{ route('accounts.deposit') }}" class="btn-secondary px-10 !py-4 !text-[11px] uppercase tracking-widest">Cancel</a>
                    </div>
                </div>
            </x-card>
        </form>
