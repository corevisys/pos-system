<x-app-layout title="Deposit List">
    <div x-data="{
        selected: [],
        allOnPage: {{ json_encode($deposits->pluck('id')) }},
        selectAll: false,
        copySuccess: false,
        toggleAll() {
            if (this.selectAll) {
        ...
        copyTable() {
            ...
            let text = 'Date\tReference No\tDebit Acc\tCredit Acc\tAmount\tCreator\n';
            rows.forEach(r => {
                ...
                if (cells.length >= 7) {
                    ...
                    let amt = cells[5].innerText.trim();
                    let creator = cells[6].innerText.trim();
                    text += `${date}\t${ref}\t${debit}\t${credit}\t${amt}\t${creator}\n`;
                }
            });
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 mb-1">
            <div>
                <h1 class="text-xl font-black tracking-tight text-text-primary dark:text-dark-text">Deposits</h1>
                <div class="flex items-center gap-2 text-text-muted font-medium mt-1">
                    ...
                    <span class="text-text-secondary text-[10px] font-bold uppercase tracking-widest">Deposit List</span>
                </div>
                <p class="text-[10px] text-text-muted font-bold uppercase tracking-widest mt-1 px-1">View/Search Deposits</p>
            </div>

            <a href="{{ route('accounts.deposit.add') }}" class="btn-primary w-full md:w-auto">
        ...
        <!-- FILTERS -->
        <form action="{{ route('accounts.deposit') }}" method="GET" class="card p-4">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <!-- Deposit Date -->
                <div class="group relative">
                    <label class="absolute -top-2 left-4 bg-card dark:bg-dark-card px-2 text-[8px] font-black uppercase text-text-muted tracking-widest z-10 transition-colors group-focus-within:text-primary">Deposit Date</label>
                    <div class="relative">
                        ...
                        <input type="date" name="deposit_date" value="{{ request('deposit_date') }}" class="input-base !pl-10 !text-[10px] !font-bold">
                    </div>
                </div>
        ...
        <div x-show="selected.length > 0" x-cloak class="p-3 bg-danger-light dark:bg-rose-950/30 border border-danger/20 dark:border-rose-800/40 rounded-card flex items-center justify-between transition-all">
            <div class="flex items-center gap-3">
                ...
                <span class="text-[11px] font-bold text-danger dark:text-rose-300">deposit(s) selected on this page</span>
            </div>
            <form action="{{ route('accounts.deposit.bulk-delete') }}" method="POST" onsubmit="return confirm('Reverse and delete all selected deposits? Destination accounts must have sufficient balances.');">
                @csrf
            ...
            <div class="px-4 py-3 border-b border-border dark:border-dark-border bg-background/40 dark:bg-white/5 flex flex-wrap justify-between items-center gap-3">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    <div class="flex bg-background dark:bg-slate-800 rounded-xl p-1 border border-border dark:border-dark-border">
                        ...
                        <a href="{{ route('accounts.deposit', array_merge(request()->query(), ['export' => 'csv'])) }}" class="px-2.5 py-1 text-[8px] font-black uppercase tracking-widest text-primary hover:bg-white dark:hover:bg-dark-card rounded-lg transition-all flex items-center">Excel</a>
                        <a href="{{ route('accounts.deposit', array_merge(request()->query(), ['export' => 'pdf'])) }}" target="_blank" class="px-2.5 py-1 text-[8px] font-black uppercase tracking-widest text-primary hover:bg-white dark:hover:bg-dark-card rounded-lg transition-all flex items-center">PDF</a>
                    </div>
                    <form method="GET" action="{{ route('accounts.deposit') }}" class="relative group">
                        @if(request('per_page'))
                            <input type="hidden" name="per_page" value="{{ request('per_page') }}">
                        @endif
                        @if(request('deposit_date'))
                            <input type="hidden" name="deposit_date" value="{{ request('deposit_date') }}">
                        @endif
                ...
                <table class="w-full text-left">
                    <thead class="bg-background dark:bg-dark-bg border-b border-border dark:border-dark-border">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-[9px] font-black text-text-muted uppercase tracking-widest">Debit Acc (Source)</th>
                            <th class="px-4 py-3 text-[9px] font-black text-text-muted uppercase tracking-widest">Credit Acc (Destination)</th>
                        ...
                        @forelse($deposits as $dep)
                            <tr class="hover:bg-slate-50/30 dark:hover:bg-slate-800/30 transition-colors group">
                                <td class="px-3 py-2 text-center">
                                    <input type="checkbox" :value="{{ $dep->id }}" x-model="selected" class="w-3 h-3 rounded border-border text-primary focus:ring-primary cursor-pointer">
                                </td>
                                <td class="px-4 py-2 text-[10px] font-black text-text-primary dark:text-dark-text leading-none">
                                    {{ \Carbon\Carbon::parse($dep->deposit_date)->format('d-m-Y') }}
                                </td>
                                ...
                                <td class="px-4 py-2 text-center">
                                    <x-dropdown align="right" width="40">
                                        ...
                                        <x-slot name="content">
                                            <x-dropdown-link :href="route('accounts.deposit.edit', $dep->id)">Edit</x-dropdown-link>
                                            <form action="{{ route('accounts.deposit.delete', $dep->id) }}" method="POST" onsubmit="return confirm('Delete and reverse this deposit? Destination account must have sufficient balance.');">
                                                @csrf
                                                @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-8 text-center">
                                    <p class="text-[9px] font-black text-text-muted uppercase tracking-widest italic leading-none">No deposits found</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
