...
**Agreed target role model (business-owner decision, 2026-09-13):** three roles — **Branch Admin** (own branch only; the `is_super_admin` flag is to be cleared on these roles), **Owner** (all stores + consolidated reports, implemented as a normal named role with cross-store reads, not a bypass-everything super-admin) and **Developer** (a separate system/maintenance super-admin account, never shared with or used as the Owner login, which may be restricted/disabled later). Access is **binary** (one branch, or all branches as Owner) — no `user_store` pivot. Details in [`MULTISTORE_GAP_ANALYSIS.md`](docs/MULTISTORE_GAP_ANALYSIS.md).

**Net security verdict:** the transactional surface is server-side gated. The remaining authorization risk is that the three per-store administrators are currently globally privileged (ISSUE-8), which the target role model above resolves.
...
**Summary:** "store" means a **tenant** (separate data partition) and "warehouse" means a **stock bucket within a tenant**. One store (a business) may operate multiple physical outlets; each outlet is modelled as its own `db_warehouse` within that store, and stock moves between outlets via the existing warehouse-to-warehouse transfer. Multiple stores already coexist in the schema and seed data (Store 1 Dhaka, Store 2 Chittagong, Store 3 Sylhet — [`StoreSeeder.php`](database/seeders/StoreSeeder.php:16)), but operational routing is single-store-per-user.
...
| ISSUE-4 | **Confirmed dead code:** `db_stocktransfer.to_store_id` / `db_stocktransferitems.to_store_id` remain in fillable but are never written by any controller. Inter-store transfer is **not** a product requirement (outlets within one store use the existing warehouse-to-warehouse transfer), so these columns are safe to drop. | [`DbStockTransfer.php`](app/Models/DbStockTransfer.php:17), [`DbStockTransferItems.php`](app/Models/DbStockTransferItems.php:18) | Low |
...
| ISSUE-8 | **Three seeded per-store administrators are globally privileged** (`is_super_admin = true` from the one-time backfill), so they bypass `EnsureUserHasStore` and every permission gate. *(To be fixed: the branch-admin roles will have this flag cleared, and cross-store visibility will move to a dedicated Owner role — see the role model in [`MULTISTORE_GAP_ANALYSIS.md`](docs/MULTISTORE_GAP_ANALYSIS.md) §4.)* | [`2026_09_12_000002`](database/migrations/2026_09_12_000002_add_is_super_admin_to_db_roles_table.php:36), [`AdminUserSeeder.php`](database/seeders/AdminUserSeeder.php:68) | High |
